...
// This function is used update the wishlist modal
function showWishItems(source) {
    ...
    if (wishList == null || wishList.length == 0) {
        ...
        if (source == 'init') { // If the wishlist is being created for the first time
        ...
        } else if (source == 'add') { // If the wishlist is being updated from the add button on the card, add the new item to the wishlist
            document.querySelector('.wish-empty').style.display = 'none';

            var item = document.createElement('div');
            var img = data[data.length - 1][3].replace("//", '/');
            item.classList.add('grid-margin', 'col-md-6', 'col-xl-6', 'wish-item-card');
            item.title = 'Click to use this image';
            item.innerHTML = `
